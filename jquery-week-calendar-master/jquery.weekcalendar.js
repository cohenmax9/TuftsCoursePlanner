...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      _create : function() {
         ...
         self._scrollToHour(8);   //set calendar at 8am at the top
...
/*
      ...
      */
      ...
      _renderEvents : function (data, $weekDayColumns) {
         ...
         if ($.isArray(data)) {
            ...
            data.events = fixDateObjects(data.events);    //Added this function to adjust the Javascript Date Objects that do not get serialized when posted
      ...
      _renderEvent: function (calEvent, $weekDay) {
         ...
         eventHtml = "<div class=\"" + eventClass + " ui-corner-all\">\
                ...
                <div class=\"wc-dept_course_num\"></div>\
                <div class=\"wc-instructor\"></div>\
                <div class=\"wc-room\"></div></div>";


         $calEvent = $(eventHtml);
         $modifiedEvent = options.eventRender(calEvent, $calEvent);

         //$("." + eventClass).css({"vertical-align": "middle", "display": "inline-block", "line-height": "20px"});

         $calEvent = $modifiedEvent ? $modifiedEvent.appendTo($weekDay) : $calEvent.appendTo($weekDay);
         $calEvent.css({lineHeight: (options.timeslotHeight - 8) + "px", fontSize: (options.timeslotHeight / 2) + "px"});
      ...
      _refreshEventDetails : function(calEvent, $calEvent) {
         ...
         $calEvent.find(".wc-dept_course_num").html(calEvent.dept_course_num);
         $calEvent.find(".wc-instructor").html(calEvent.instructor);
         $calEvent.find(".wc-room").html(calEvent.room);
   ...
   function fixDateObjects(schedule) {    //This function creates a new object with the Javascript Date Objects corrected (orignally show up as 1/1/2014)
      var fixed = new Array();
      for (var i = 0; i < schedule.length; i++) {
         fixed[i] = new Object();
         fixed[i].start = new Date(schedule[i].start);
         fixed[i].end = new Date(schedule[i].end);
         fixed[i].title = schedule[i].title;
         fixed[i].dept_course_num = schedule[i].dept_course_num;
         fixed[i].room = schedule[i].room;
         fixed[i].instructor = schedule[i].instructor;
         fixed[i].id = schedule[i].id;
         fixed[i].course_id = schedule[i].course_id;
      }
